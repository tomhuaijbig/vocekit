...
3. 助手内部的 SAPI 兼容有界生产者/消费者音频流把持续输入交给 `SpeechRecognitionEngine`。
...
识别回调可能来自不同线程。助手必须用同一把锁完成“序列化、写一整行、刷新 stdout”，并显式使用无 BOM 的 UTF-8；任何两个事件都不能交错成一行。`recognized` 仍表示一个确认片段，但累计文本必须使用确定性连接规则：两个片段边界都是拉丁字母或数字时补一个空格，中文等非拉丁边界不强行插空格。流式预览和最终文本共用这条规则。
...
- stdin 生产线程遇到内部 64,000 字节队列暂满时必须等待消费者释放容量，而不是把正常的管道背压误报为识别失败；取消和 EOF 必须唤醒两侧等待者。Qt 侧仍保留独立的两秒待写上限，只有采集速度长期超过助手消费速度时才触发降级。

### System.Speech 流兼容约束

实施前的本机方法级探针确认，`System.Speech` 在绑定 `SetInputToAudioStream` 时会读取 `Length`、查询当前位置并执行 `Seek(0, Current)`。完全不可 seek 的持续流会抛出 `NotSupportedException`；在绑定时报告长度 0 又会被立即视为 EOF。

因此助手流必须采用经过真实 zh-CN/en-US 验证的最小兼容语义：

- `CanSeek` 返回 true，但只允许查询当前位置以及不改变位置的 `Seek(0, Current)`；任何重新定位都拒绝，不能让识别器回读已经释放的音频。
- `Length` 返回固定 64 MiB 声明上限，而不是当前队列长度；当前最长 30 分钟、16 kHz、单声道、16-bit PCM 为 57,600,000 字节，位于该上限内。
- 实际内存队列仍只缓存约两秒、64,000 字节；声明长度不能导致预分配 64 MiB 或无限缓存。
- `Read` 必须尽量填满识别器请求的缓冲区；队列暂时为空时阻塞等待新 PCM，生产结束后用短读/0 表示真实 EOF。
- 如果累计输入超过声明上限，助手返回稳定协议错误并终止本次会话，不能覆盖或环回旧音频。

本机实测以 640 字节分块持续喂入合成的 zh-CN 与 en-US PCM：两者都产生 9 次 `SpeechHypothesized`、1 次 `SpeechRecognized`，并在 EOF 后以 `InputStreamEnded=true` 完成。这项探针行为必须固化为助手测试，避免以后把兼容流误改回普通非 seek 流。
...
Windows 流式会话已经收到 `ready` 后再发生进程、协议、超时或写入积压失败时，控制器继续保存完整 PCM。用户确认时再启动一个 `batch` 助手，把完整 PCM 写入后关闭 stdin：
...
若助手缺失、无法启动、启动超时、运行时不可用、识别器缺失或语法加载失败发生在 `ready` 之前，则立即停止本次录音并显示对应修复提示，不再用同一已知不可用的组件重复跑 batch。
...
- 缺少识别语言、System.Speech 运行能力或听写语法时显示明确原因和打开 Windows 语言设置的按钮；助手可执行文件缺失时提示重新安装或完整解压软件，不能误导用户去修改语言设置。
...
- 最终失败：不运行 AI 或写入，保留录音，显示与错误类型匹配的诊断和修复入口。
...
- 整段兜底：按音频时长设置有上限的超时，建议 `max(15 秒, 音频时长 x 1.5 + 10 秒)`，再受 35 分钟硬上限约束；30 分钟本地音频不能被固定 120 秒提前截断。取消仍需在最多 50 ms 的轮询周期内生效。
...
- 新助手源码位于独立 helper 目录，目标为 Windows 自带的 .NET Framework 4.x、x64、Release。构建脚本先尝试标准 .NET Framework 4.8 MSBuild；若机器只有运行时、没有 4.8 Developer Pack，则使用 VS2022 自带 Roslyn `csc.exe`，显式引用 64 位 Framework/GAC 系统程序集进行等价编译。不得因为缺少仅开发机需要的 targeting pack 而要求最终用户安装 SDK。
- 交付路径固定为 `speech/windows/vocekit-windows-speech.exe`，运行时通过应用目录解析，不依赖工作目录或 PATH。
- 构建脚本先生成助手，再执行 Qt 部署；部署和运行时验证脚本都必须检查 helper 存在并能完成不依赖语言包的 `--self-test`。真实识别器 `--probe` 属于设置页诊断和系统冒烟测试，不能把用户机器缺少某个语言包误判成安装包损坏。
...
- SAPI 兼容有界音频流在生产快于消费、EOF 和取消时不会死锁或无限增长；`Length`、`Position`、`Seek(0, Current)` 和拒绝重新定位均有回归测试。
- 内部队列暂满时生产者被背压且能在消费/取消后恢复；并发识别事件不会交错 stdout 行；中英文多片段连接不会粘词或给中文强插空格。

...
QString faqIdForTitle(const QString &title)
{
    ...
    if (textContains(title, "TLS 初始化或握手失败")) return QStringLiteral("17");
...
QString faqIdForPopup(const QString &title, const QString &text)
{
    ...
    if (textContains(combined, "TLS initialization failed")
        || textContains(combined, "TLS backend")
        || textContains(combined, "qschannelbackend.dll")) return QStringLiteral("17");

    ...
    : QWidget(parent), m_openDiagnostic(openDiagnostic)
{
        ...
        items->addWidget(faqCard(
            tr8("处理失败：TLS 初始化或握手失败"),
            tr8("弹窗可能提示 TLS 初始化失败、证书错误或安全通道无法建立。Qt 6 在 Windows 上使用随程序部署的 TLS 后端和系统证书。"),
            QStringList()
                << tr8("优先使用打包脚本生成的测试包运行，不要只复制 exe。")
                << tr8("确认程序目录里有 Qt 运行库、tls/qschannelbackend.dll 和 platforms/qwindows.dll。")
                << tr8("检查系统时间、证书、代理和安全软件是否阻止 HTTPS。")

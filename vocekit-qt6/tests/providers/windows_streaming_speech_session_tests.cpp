...
WindowsStreamingSpeechSession::Timing fastTiming()
{
    ...
    // Starting a helper process can exceed 200 ms on a busy Windows host even
    // though the fake helper emits its ready event after only 75 ms.
    timing.startupTimeoutMs = 1000;

            ...
            for (const QChar character : requiredCjk) {
                QVERIFY2(rawFont.supportsCharacter(character),
                         qPrintable(QStringLiteral("missing CJK glyph U+%1")
                             .arg(static_cast<ushort>(character.unicode()),
                                  4, 16, QLatin1Char('0'))));
            }
        }
...
executionModeSelectorStaysVisibleAcrossPureNavigation()
{
    ...
    for (QPushButton *button : textButtons) {
        QVERIFY2(
            ...
            qPrintable(QStringLiteral("%1 needs %2, has %3")
                ...
                .arg(button->fontMetrics().horizontalAdvance(button->text()) + 20)
                .arg(button->width()))
        );
        const QRect bounds(
    ...
    QVERIFY2(
        modeLabel->fontMetrics().horizontalAdvance(modeLabel->text())
            <= modeLabel->width(),
        qPrintable(QStringLiteral("mode label needs %1, has %2")
            .arg(modeLabel->fontMetrics().horizontalAdvance(modeLabel->text()))
            .arg(modeLabel->width()))
    );
    ...
    QVERIFY2(
        returnButton->fontMetrics().horizontalAdvance(returnButton->text()) + 20
            <= returnButton->width(),
        qPrintable(QStringLiteral("return button needs %1, has %2")
            .arg(returnButton->fontMetrics().horizontalAdvance(
                returnButton->text()
            ) + 20)

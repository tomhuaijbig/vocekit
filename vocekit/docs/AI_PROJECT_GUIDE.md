...
下面命令中的 Qt 和 MinGW 路径需要按自己的电脑替换：
...
```powershell
& "$env:MINGW_BIN\mingw32-make.exe" -f Makefile.Release -j2
```

启动调试版：

```powershell
Start-Process -FilePath '.\debug\vocekit.exe' -WorkingDirectory '.\debug'
```

...
QString resolvedStage(const QString &stage)
{
    const QString trimmed = stage.trimmed();
    return trimmed.isEmpty()
        ? textUtf8("\xE5\xA4\xA7\xE6\xA8\xA1\xE5\x9E\x8B")
        : trimmed;
}
...
VoiceResultCompletionResult VoiceResultCompletionExecutor::run(
    const VoiceResultCompletionRequest &request,
    const VoiceResultCompletionHandlers &handlers
)
{
    VoiceResultCompletionResult result;
    const QString stage = resolvedStage(request.failureStage);

    if (!handlers.runContext || !handlers.finalizeOutput) {
        result.error = textUtf8(
            "\xE7\xBB\x93\xE6\x9E\x9C\xE6\x89\xA7\xE8\xA1\x8C\xE5\x99\xA8\xE6\x9C\xAA\xE9\x85\x8D\xE7\xBD\xAE\xE5\xAE\x8C\xE6\x95\xB4\xE3\x80\x82"
        );
        result.logAction = textUtf8("\xE5\xA4\xB1\xE8\xB4\xA5");
        result.logDetail =
            textUtf8("\xE9\x98\xB6\xE6\xAE\xB5=") + stage
            + textUtf8("\xEF\xBC\x8C\xE5\x8A\x9F\xE8\x83\xBD=") + request.context.modeId
    ...
    if (handlers.wasCancelled && handlers.wasCancelled()) {
        ...
        result.logDetail =
            textUtf8("\xE9\x98\xB6\xE6\xAE\xB5=") + stage
            + textUtf8("\xEF\xBC\x8C\xE5\x8A\x9F\xE8\x83\xBD=")
